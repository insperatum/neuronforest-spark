 ...
 */
@Experimental
trait MyImpurity extends Serializable {
 ...
 */
private[tree] abstract class MyImpurityAggregator(val statsSize: Int) extends Serializable {
   ...
   */
  def update(allStats: Array[Double], offset: Int, label: Double3, instanceWeight: Double): Unit
   ...
   * @param offset    Start index of stats for this (node, feature, bin).
   */
  def getCalculator(allStats: Array[Double], offset: Int): MyImpurityCalculator

}

/**
 * Stores statistics for one (node, feature, bin) for calculating impurity.
 * Unlike [[MyImpurityAggregator]], this class stores its own data and is for a specific
 * (node, feature, bin).
 * @param stats  Array of sufficient statistics for a (node, feature, bin).
 */
private[tree] abstract class MyImpurityCalculator(val stats: Array[Double]) {

  /**
   * Make a deep copy of this [[MyImpurityCalculator]].
   */
  def copy: MyImpurityCalculator
   ...
   */
  def add(other: MyImpurityCalculator): MyImpurityCalculator = {
    require(stats.size == other.stats.size,
      s"Two MyImpurityCalculator instances cannot be added with different counts sizes." +
   ...
   * calculator.
   */
  def subtract(other: MyImpurityCalculator): MyImpurityCalculator = {
    require(stats.size == other.stats.size,
      s"Two MyImpurityCalculator instances cannot be subtracted with different counts sizes." +
      s"  Sizes are ${stats.size} and ${other.stats.size}.")
    var i = 0
    while (i < other.stats.size) {
      stats(i) -= other.stats(i)
   ...
   */
  def prob(label: Double3): Double = -1
  ...
  protected def indexOfLargestArrayElement(array: Array[Double]): Int = {
    ...
    if (result._1 < 0) {
      throw new RuntimeException("MyImpurityCalculator internal error:" +
