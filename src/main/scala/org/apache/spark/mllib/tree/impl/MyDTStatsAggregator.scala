 ...
 */
private[tree] class MyDTStatsAggregator(
    val metadata: MyDecisionTreeMetadata,
    featureSubset: Option[Array[Int]]) extends Serializable {

  /**
   * [[MyImpurityAggregator]] instance specifying the impurity type.
   */
  val impurityAggregator: MyImpurityAggregator = metadata.impurity match {
    //case Gini => new GiniAggregator(metadata.numClasses)
    //case Entropy => new EntropyAggregator(metadata.numClasses)
    case MyVariance => new MyVarianceAggregator()
  ...
  /**
   * Get an [[MyImpurityCalculator]] for a given (node, feature, bin).
   ...
   */
  def getImpurityCalculator(featureOffset: Int, binIndex: Int): MyImpurityCalculator = {
   ...
   */
  def update(featureIndex: Int, binIndex: Int, label: DoubleTuple, instanceWeight: Double): Unit = {
   ...
   */
  def merge(other: MyDTStatsAggregator): MyDTStatsAggregator = {
    require(allStatsSize == other.allStatsSize,
      s"MyDTStatsAggregator.merge requires that both aggregators have the same length stats vectors."
